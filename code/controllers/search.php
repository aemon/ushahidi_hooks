...
class Search_Controller extends Main_Controller {
	...
	public function index($page = 1)
	{
		...
		$stop_words = array('the', 'and', 'a', 'to', 'of', 'in', 'i', 'is', 'that', 'it',
			...
			'at', 'or', 'as', 'was', 'so', 'if', 'out', 'not', 'не', 'нет', 'был', 'были', 'или', 'и', 
			'вы', 'как', 'но', 'оно', 'это', 'на', 'в', 'я', 'есть', 'быть', 'для'
		);
		...
		if ( ! empty($search_query))
		{
			...
			foreach ($query as $search)
			{
				...
				$incident_date = date('H:i M d, Y', strtotime($search->incident_date));

				$location_id = $search->location_id;
				$location_names = ORM::Factory('location')->where('id', $location_id)->find();
				$location_name = $location_names->location_name;
				
				$incident_verified = $search->incident_verified;
...
//				$html .= "</div>";
				...
				$html .= "<div id=\"incident_" . $incident_id . "\" class=\"rb_report " . $incident_verified_class . "\"><div class=\"r_media\"><p class=\"r_photo\"> <a href=\"" . $incident_url . "\"><img alt=\"" . $incident_description . "\" src=\"" . $incident_thumb . "\" /> </a></p>";
				$html .= "<p class=\"r_video\" style=\"display:none;\"><a href=\"#\">" . Kohana::lang('ui_main.video') . "</a></p></div>";

				$html .= "<div class=\"r_details\"><h3><a class=\"r_title\" href=\"" . $incident_url . "\">" . html::escape($incident_title) . "</a>" . $incident_verified . "</h3>";
				$html .= "<p class=\"r_date2 bottom-cap\">" . $incident_date . "</p><div class=\"r_description\">" ;
				$html .=  $highlight_description;
				$html .= "</div>";				
				$html .= "<p class=\"r_location\"><a href=\"" . url::site("reports/?l=$location_id") . "\">" . html::specialchars($location_name) . "</a></p>";
				$html .= "</div></div>";
			}
		}
		else
